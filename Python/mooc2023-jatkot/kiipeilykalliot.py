...
def jarjestys_reitit(alkio: Kiipeilykallio):
    return alkio.reitteja()

def reittien_maaran_mukaan(alkio: Kiipeilykallio):
    return sorted(alkio, key=jarjestys_reitit)

def jarjestys_vaikeus(alkio: Kiipeilykallio):
    return alkio.vaikein_reitti().grade

def vaikeimman_reitin_mukaan(alkio: Kiipeilykallio):
    return sorted(alkio, key=jarjestys_vaikeus, reverse=True)
